...
function Home() {
    ...
    useEffect(() => {
        ...
        async function asyncUseEffect() {
            ...
            const OrganizerContract = new context.library.eth.Contract(
                OrganizerAbi,
                _onTestnet ? KovanOrganizerAddr : MainnetOrganizerAddr
            );

            OrganizerContract.methods
                .varianceSwapInstancesLength()
                .call()
                .then(async (res: string) => {
                    ...
                    setInstances(
                        results.map((address: string, index: number) => (
                            <VarSwapInfo
                                address={address}
                                link={true}
                                key={index}
                ...
                <div className="home-content">
                    <div className="home-jumbotron">
                        <div className="jumbotron">
                            <h1>Decentralised Volatilty Trading</h1>
                            <p>
                                Allows counterparties to make
                    ...
                    <h1 className="variance-header">Variance Swaps</h1>
                    <ul className="productList">{instances}</ul>
                </div>
            </div>
        );
    } else {
